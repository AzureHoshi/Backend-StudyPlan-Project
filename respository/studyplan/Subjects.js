...
async function getSubjects(curriculum_id, subject_id) {
    ...
    return new Promise((resolve, reject) => {
        ...
        WHERE subjects.curriculum_id = ${curriculum_id} AND ${table}.is_deleted IS NULL ORDER BY ${table}.subject_code DESC`
        }
        else {
            Query = `SELECT * FROM ${table} 
        INNER JOIN curriculums ON ${table}.curriculum_id = curriculums.curriculum_id
        INNER JOIN group_type ON ${table}.group_type_id = group_type.group_type_id
        WHERE subjects.subject_id = ${subject_id} AND ${table}.is_deleted IS NULL`
        }
...
async function getSubjectsGroupType(curriculum_id, group_type_id) {
    ...
    return new Promise((resolve, reject) => {
        ...
        INNER JOIN curriculums ON ${table}.curriculum_id = curriculums.curriculum_id
        INNER JOIN group_type ON ${table}.group_type_id = group_type.group_type_id
        WHERE subjects.group_type_id = ${group_type_id} AND subjects.curriculum_id = ${curriculum_id} AND ${table}.is_deleted IS NULL`
...
async function getSubjectsSubjectType(curriculum_id, subject_type_id) {
    ...
    return new Promise((resolve, reject) => {
        ...
        WHERE group_type.subject_type_id = ${subject_type_id} AND subjects.curriculum_id = ${curriculum_id} AND ${table}.is_deleted IS NULL`
...
async function getSubjectsCategory(curriculum_id, subject_category_id) {
    ...
    return new Promise((resolve, reject) => {
        ...
        WHERE subject_type.subject_category_id = ${subject_category_id} AND subjects.curriculum_id = ${curriculum_id} AND ${table}.is_deleted IS NULL`
...
async function addNewSubject(curriculum_id, group_type_id, subject_code, subject_name_th, subject_name_en, credit_qty, subject_description) {
...
async function editSubject(subject_id, group_type_id, subject_code, subject_name_th, subject_name_en, credit_qty, subject_description) {
...
async function searchSubject(text, column, curriculum_id) {
    ...
    var Query = `SELECT * FROM ${table} 
        INNER JOIN curriculums ON ${table}.curriculum_id = curriculums.curriculum_id
        INNER JOIN group_type ON ${table}.group_type_id = group_type.group_type_id
        WHERE ${table}.is_deleted IS NULL AND ${column} LIKE '%${text}%' AND curriculums.curriculum_id = ${curriculum_id} ORDER BY ${table}.subject_code DESC`
...
async function getCategories(category_id) {
    ...
    return new Promise((resolve, reject) => {
        if (category_id !== '') {
            Query = `SELECT * FROM subject_category WHERE subject_category_id = ${category_id} AND is_deleted IS NULL ORDER BY subject_category_id DESC`
        } else {
            Query = `SELECT * FROM subject_category WHERE is_deleted IS NULL ORDER BY subject_category_id DESC`
        }
        console.log('Query1 is: ', Query);
...
async function editCategory(subject_category_id, subject_category_name) {
    ...
    return new Promise((resolve, reject) => {
        Query = crud.UpdateById('subject_category', 'subject_category_id', subject_category_id)
...
async function deleteCategory(subject_category_id) {
    ...
    return new Promise((resolve, reject) => {
        Query = crud.Delete('subject_category', 'subject_category_id', subject_category_id)
...
async function searchCategories(text, column) {
    ...
    var Query = `SELECT * FROM subject_category 
        WHERE subject_category.is_deleted IS NULL AND ${column} LIKE '%${text}%' ORDER BY subject_category_id DESC`
    ...
    return new Promise((resolve, reject) => {
        if (subject_type_id !== '' && subject_category_id === '') {
            Query = `SELECT * FROM subject_type 
             INNER JOIN subject_category ON subject_type.subject_category_id = subject_category.subject_category_id
            WHERE subject_type_id = ${subject_type_id} AND subject_type.is_deleted IS NULL ORDER BY subject_type_id DESC`
        } else if (subject_category_id !== '') {
            Query = `SELECT * FROM subject_type 
             INNER JOIN subject_category ON subject_type.subject_category_id = subject_category.subject_category_id
            WHERE subject_type.subject_category_id = ${subject_category_id} AND subject_type.is_deleted IS NULL ORDER BY subject_type_id DESC`
        }
        else {
            Query = `SELECT * FROM subject_type 
            INNER JOIN subject_category ON subject_type.subject_category_id = subject_category.subject_category_id
            WHERE subject_type.is_deleted IS NULL ORDER BY subject_type_id DESC`
        }
        console.log('Query1 is: ', Query);
        ...
        pool.query(Query, function (error, results1, fields) {
            ...
            } else if (results1.length > 0 && subject_type_id && !subject_category_id) {
                pool.end();
                return resolve({
                    statusCode: 200,
                    returnCode: 1,
            ...
            } else if (subject_category_id !== '' && subject_category_id && !subject_type_id) {
                pool.end();
                return resolve({
                    statusCode: 200,
                    returnCode: 1,
...
async function searchSubjectType(text, column) {
    ...
    var Query = `SELECT * FROM subject_type
        INNER JOIN subject_category ON subject_type.subject_category_id = subject_category.subject_category_id
        WHERE subject_type.is_deleted IS NULL AND ${column} LIKE '%${text}%' ORDER BY subject_type.subject_type_id DESC`
...
async function getGroupType(curriculum_id) {
    ...
    return new Promise((resolve, reject) => {
        ...
        ON a.group_type_id = group_type.group_type_id WHERE group_type.is_deleted IS NULL ORDER BY a.group_type_id DESC`
...
async function getGroupType2(group_type_id, subject_type_id) {
    ...
    return new Promise((resolve, reject) => {
        if (group_type_id !== '' && subject_type_id === '') {
            ...
            WHERE group_type_id = ${group_type_id} AND group_type.is_deleted IS NULL ORDER BY group_type_id DESC`
        } else if (subject_type_id !== '' && group_type_id === '') {
            Query = `SELECT * FROM group_type
            INNER JOIN subject_type ON subject_type.subject_type_id = group_type.subject_type_id
            WHERE group_type.subject_type_id = ${subject_type_id} AND group_type.is_deleted IS NULL ORDER BY group_type_id DESC`
        } else {
            Query = `SELECT * FROM group_type 
            INNER JOIN subject_type ON subject_type.subject_type_id = group_type.subject_type_id
            WHERE group_type.is_deleted IS NULL ORDER BY group_type_id DESC`
        }
        console.log('Query1 is: ', Query);
        ...
        pool.query(Query, function (error, results1, fields) {
            ...
            if (results1.length > 0 && group_type_id !== '' && subject_type_id === '') {
                pool.end();
                return resolve({
                    statusCode: 200,
                    returnCode: 1,
            ...
            } else if (results1.length > 0 && subject_type_id !== '' && group_type_id === '') {
                pool.end();
                return resolve({
                    statusCode: 200,
                    returnCode: 1,
            ...
            } else if (results1.length > 0 && subject_type_id === '' && group_type_id === '' || results1.length > 0 && subject_type_id !== '' && group_type_id !== '') {
                pool.end();
                return resolve({
                    statusCode: 200,
                    returnCode: 1,
...
async function searchGroupType(text, column) {
    ...
    var Query = `SELECT * FROM group_type
        INNER JOIN subject_type ON subject_type.subject_type_id = group_type.subject_type_id
        WHERE group_type.is_deleted IS NULL AND ${column} LIKE '%${text}%' ORDER BY group_type.group_type_id DESC`
